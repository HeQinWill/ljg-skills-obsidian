...
2. **生成 YAML 属性 (Frontmatter)**:
   ...
   aliases:
     - "Invest: {项目名称}"
   skill: ljg-invest
   status: {建议: 建议投资/建议观察/建议放弃}
   ---
   ```
3. **写入路径**：`~/Obsidian/aitalk/{时间戳}--投资分析-{项目名称}__invest.md`
4. **Markdown 格式与双向链接**:
   - 使用标准 Markdown 语法。
   - 对 **项目名称**、**竞争对手**、**核心赛道概念**、**创生公式关键组件** 使用 `[[维基链接]]`。
   - 使用 `> [!todo] 未解问题` 和 `> [!quote] 核心假设` Callouts。
5. **向用户报告文件路径**
