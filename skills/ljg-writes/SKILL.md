...
## 输出
从观点中提取关键词作为标题,指定 Obsidian 输出或，遵循此规范：
...
2. **生成 YAML 属性 (Frontmatter)**:
   ...
   ---
   ```
3. **写入路径**：`~/Obsidian/aitalk/{时间戳}--{标题关键词}__write.md`
4. **Markdown 格式与双向链接**:
   - 使用标准 Markdown 语法（禁止子标题，保持线性流）。
   - 在正文中对 **核心观点**、**跨域类比的对象**、**引用的计算机体系概念** 使用 `[[维基链接]]`。
   - 鼓励链接到库中已有的相关思考笔记，形成知识网。
5. **向用户报告文件路径**