...
2. **生成 YAML 属性 (Frontmatter)**:
   ...
   ---
   ```
3. **写入路径**：`~/Obsidian/aitalk/{时间戳}--{城市}旅行研究__travel.md`
4. **Markdown 格式与双向链接**:
   - 对 **博物馆名称**、**古建筑名称**、**核心文物**、**历史人物** 强制使用 `[[维基链接]]`。
   - 使用 `> [!tip] 重点看` 和 `> [!example] 路线建议` Callouts。
5. **向用户报告文件路径**


将步骤 2（研究结果）和步骤 3（内容提炼，如有）合成为一份结构化 md 文档。
